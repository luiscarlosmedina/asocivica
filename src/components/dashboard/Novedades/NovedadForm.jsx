...
export default function NovedadForm({ onDataUpdate }) {
  ...
  const fetchDataTpnoedad = () => {
    fetch(
      "http://20.106.206.47/api_sisinov/public/api/tpnov"
    )
  ...
  const fetchDataEmpresa = () => {
    fetch(
      "http://20.106.206.47/api_sisinov/public/api/novedadempresa"
    )
      .then((response) => response.json())
      .then((empresa) => {
  ...
  const fetchDataSede = (id_e) => {
    fetch(
      `http://20.106.206.47/api_sisinov/public/api/novedadsede/${id_e}`
    )
      .then((response) => response.json())
      .then((sede) => {
        setSede(sede.data);
  ...
  const fetchDataEmpleado = () => {
    fetch(
      "http://20.106.206.47/api_sisinov/public/api/novedadempleados"
    )
      .then((response) => response.json())
      .then((empleado) => {

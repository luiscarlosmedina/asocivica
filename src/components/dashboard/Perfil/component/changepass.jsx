...
export default function Changepass({ id }) {
    ...
    const handleVerif = async () => {
        try {
            ...
            const response = await fetch(`http://20.106.206.47/api_sisinov/public/api/verifpass`, {
                method: 'POST',
                headers: {
    ...
    const cambiar = async () => {
        try {
            ...
            if (user.id_em === id) {
                const ress = await fetch(`http://20.106.206.47/api_sisinov/public/api/changepass`, {
                    method: 'POST',
                    headers: {
                        'Content-Type': 'application/json',
                    },
